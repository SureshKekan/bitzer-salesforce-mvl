...
/*
$Id: nusoap.php,v 1.2 2005/10/27 20:37:19 ryankicks Exp $
...
* @version  $Id: nusoap.php,v 1.2 2005/10/27 20:37:19 ryankicks Exp $
* @access   public
*/
class nusoap_base {
	/**
	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded'){
        ...
        // RCHOI: serialize special array as repeated elements
        if (is_object($val) && get_class($val) == 'repeatedelementsarray'){
...
class wsdl extends nusoap_base {
	...
	*/
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
        ...
        if (is_object($value) && (
	     get_class($value) == 'sobject' ||
	     get_class($value) == 'leadconvert')){
		...
		if(isset($this->body_position) && $pos > $this->body_position){
            ...
            // build complexType values
				...
				if(!isset($this->message[$pos]['result'])){
                    ...
                    $attrs = $this->message[$pos]['attrs'];
                    $type = isset($attrs['xsi:type']) ? $attrs['xsi:type'] : null;
                    $foundSObject = strpos($type, 'sObject');
                    
                    // RCHOI: specifically looking for type sObject
                    if ($foundSObject){
                        $recordArray = $this->buildVal($pos);
                        $sobj = new sObject($this->buildVal($pos));
                        $this->message[$pos]['result'] = $sobj;
                    } else {    
